    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (user != null)
            {
                ...
                {
                    ModelState.AddModelError("", "Musisz potwierdzić email żeby się zalogować.");
                    return View();
                }
            }
        ...
        public async Task<ActionResult> ExternalLoginCallback(string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                default:
                    ...
                    // userName@steam.nasze.com
                    //if (loginInfo.Email == null)
                    //{
                    //    const string chars = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
                    //    int length = 10;
                    //    var random = new Random();
                    //    string uname = new string(Enumerable.Repeat(chars, length).Select(s => s[random.Next(s.Length)]).ToArray());
                    //    //string upass = new string(Enumerable.Repeat(chars, length).Select(s => s[random.Next(s.Length)]).ToArray());
                    //    //string umail = uname + "@gmail.com";
                    //    var visitor = new ApplicationUser { UserName = uname, Email = null };
                    //    await UserManager.CreateAsync(visitor, null);
                    //    await SignInManager.SignInAsync(visitor, isPersistent: false, rememberBrowser: false);
                    //    return RedirectToAction("Index", "Home");
                    //}
                    //else
                    //{ 
